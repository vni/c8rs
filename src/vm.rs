...
pub struct Chip8 {
    ...
    pc: u16,
    sp: u8,
    stack: [u16; 16],        //??32 // TODO: should be part of the memory
...
impl Chip8 {
    ...
    pub fn process_instruction(&mut self) {
        ...
        // ui::Terminal::print_state(&chip, &op);
        ...
        match (a, b, c, d) {
            ...
            (0xc, x, a, b) => {
                chip.regs[x as usize] = rand::thread_rng().gen::<u8>() & byte(a, b);
            ...
            (0xe, x, 9, 0xe) => {
                if Chip8::key_pressed(chip.regs[x as usize]) {
                    chip.pc += 2;
                }
            }
            ...
            (0xf, x, 0, 0xa) => {
                chip.regs[x as usize] = Chip8::wait_key_press();
            }
